...
class WebpackRequireFrom {
  constructor (userOptions) {
    ...
    if (this.options.methodName && this.options.path) {
      throw new Error(`${PLUGIN_NAME}: Specify either "methodName" or "path". See https://github.com/agoldis/webpack-require-from#configuration`)
    }
  }
  ...
  compilationHook (compilation, params) {
    getHookMethod(compilation.mainTemplate, 'require-extensions')((source, chunk, hash) => {
      ...
      if (_config.methodName) {
        ...
        throw new Error(`${PLUGIN_NAME}: cannot determine what method to use. See https://github.com/agoldis/webpack-require-from#configuration`)
      }
      return [
        source,
        `// ${PLUGIN_NAME}`,
